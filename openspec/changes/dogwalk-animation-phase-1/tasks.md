...
- [x] 4.2 Cook skinned Final (GPU bone palette / skinning shader inputs)
- [x] 4.3 GPU skinned draw for fresh Final; Editor Final and Player share path
- [x] 4.4 Pose parity test: one frame CPU vs GPU (or CPU vs reference) within tolerance

## 5. Edit Mode preview

- [x] 5.1 Authorship viewport controls to Play/Pause/Stop/Loop AnimationPlayer without DotNetHost
- [x] 5.2 Verify Behaviour Tick does not run on Edit preview path
...
- [x] 6.3 Play acceptance: test-rig idle↔walk hard cut + deformation (engineering gate)
