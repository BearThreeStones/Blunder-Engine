...
The editor SHALL provide a reusable Slint component that draws a close (×) icon using vector geometry (not text glyphs). The icon SHALL be legible at 9–12 px effective size on dark dock backgrounds and SHALL accept a `color` property for default and hover styling by parent `TouchArea`.
