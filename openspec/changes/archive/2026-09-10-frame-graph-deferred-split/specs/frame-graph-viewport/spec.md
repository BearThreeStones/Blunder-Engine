...
**Reason**: Editor Deferred is a G-buffer Pass then a Lighting Pass, not one Scene wrapping `renderFrame`.
**Migration**: `recordViewportGraph` adds those two Passes when the editor uses Deferred. Shadow stays in the G-buffer callback. LOAD stays in the Lighting callback. G-buffer images stay path-owned.
