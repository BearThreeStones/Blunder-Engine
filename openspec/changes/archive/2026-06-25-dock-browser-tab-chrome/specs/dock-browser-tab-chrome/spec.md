...
Dragging a tab in the integrated chrome row SHALL be the primary gesture to detach, show drag preview, commit OS float, and re-dock with dock guides; existing `DockManager` tab-drag semantics SHALL be preserved.

#### Scenario: Tab drag from OS float starts dock drag

- **WHEN** the user presses and drags a tab in the OS float chrome row
...
- **WHEN** the user presses and drags a tab in a main-window dock container chrome row
