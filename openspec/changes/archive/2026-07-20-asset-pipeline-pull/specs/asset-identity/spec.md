...
The Project SHALL keep four content roles: Assets root for Intermediate descriptors (and Scene documents); Resources root (non-Source) for Intermediate data bodies; Source root (`Resources/Source/`) for Source Assets only; Cooked cache (`.blunder/cooked/`) for Final Assets keyed by GUID.
...
- **WHEN** Import performs Source Export from FBX or OBJ
- **THEN** the original file is stored under the Source root and Intermediate glTF under Resources (non-Source)
