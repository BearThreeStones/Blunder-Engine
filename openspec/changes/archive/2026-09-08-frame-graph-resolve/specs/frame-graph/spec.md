...
`resolvedTexture` and `resolvedBuffer` SHALL return the non-owning RHI object attached to a Frame graph handle. Both queries SHALL be Setup state: they SHALL NOT require compile. DCE SHALL NOT erase the pointer. A never-created handle, a Transient resource in this slice, a shape mismatch, or a null import SHALL return null and SHALL NOT throw. Resolve SHALL NOT allocate GPU memory. Tests SHALL use dummy RHI stand-ins and SHALL NOT create a Vulkan device.
...
- **WHEN** a test queries `resolvedTexture` or `resolvedBuffer` on a never-created handle, a Transient, or the wrong shape for that handle
