...
Host edge dock drop detection SHALL use only the host guide icon rectangle. Full-edge bands SHALL NOT activate host edge dock.

#### Scenario: Pointer on icon activates host dock target

- **WHEN** the pointer is inside the right host guide icon bounds
...
- **WHEN** the pointer is within the auto-hide edge zone on the right but not over the host guide icon
...
Host edge guide icons SHALL use the mini-window pictographic style (blue frame, chrome bar, half-pane fill) consistent with split cross guides, not the auto-hide flat-wide arrow icon style.

#### Scenario: Host guide distinct from auto-hide

- **WHEN** host edge guide icons are visible during drag
- **THEN** they SHALL NOT use the auto-hide perimeter flat-wide icon with inward arrow
- **AND** they SHALL be visually consistent with `DockGuideButton` split cross icons
