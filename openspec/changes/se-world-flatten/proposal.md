...
- **~4795** LI/PR layout instances (196 library assets) as entities with **shared** Mesh Asset GUID refs, plus set grouping nodes and set-glTF **GEO** (ground / path / snow / pond / creek). Library-internal `instance_asset_id` (needles, knots, leaves) expand **under** that layout instance. **COL-*** get **no MeshRenderer** this slice and are **not spawned** (no `active: false` placeholders).
