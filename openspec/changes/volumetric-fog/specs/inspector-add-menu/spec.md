...
The Inspector SHALL provide a single **Add…** picker for the current selection. Unique attachments SHALL be Camera, Light, Fog, Skeleton, AnimationPlayer, and AnimationTree. Behaviour types from the Behaviour type catalog and SkeletonModifier types remain listed. Mesh SHALL remain Content Browser spawn and SHALL NOT appear in Add…. The picker SHALL be a grouped flat list (Unique attachments, then Behaviours, then Skeleton Modifiers) with no search. An empty Behaviour catalog SHALL still show the Behaviours group with the existing build-Scripts hint.

#### Scenario: Open Add… on one entity
- **WHEN** exactly one entity is selected and the author opens Add…
- **THEN** the picker lists Unique attachments (including Light and Fog), Behaviour types (or the build-Scripts hint), and SkeletonModifier types, grouped in that order
...
Camera, Light, Fog, Skeleton, AnimationPlayer, and AnimationTree SHALL exist at most once on the selected Object or entity. When a Unique attachment is already present, its Add… row SHALL stay visible and disabled. Behaviours and SkeletonModifiers SHALL remain addable (multiple allowed).
...
#### Scenario: Fog already present
- **WHEN** the selected entity has a Fog Component and the author opens Add…
- **THEN** the Fog row is visible and cannot be chosen
...
Adding Skeleton, AnimationPlayer, AnimationTree, a Behaviour, or a SkeletonModifier SHALL create a bound Object when the selected entity has none. Adding Camera, Light, or Fog SHALL NOT create an Object.
...
#### Scenario: Add Fog on mesh-only entity
- **WHEN** the selected entity has no bound Object and the author adds Fog
- **THEN** the entity has a Fog Component and still has no bound Object
