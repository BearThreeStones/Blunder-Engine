...
- Define `project.blunder` + Create scaffold; user Projects under `E:/Blunder Projects` (engine checkout is not a Project)
...
8. **Projects directory** — Canonical parent for Create is `E:/Blunder Projects` on Windows (`defaultProjectsDirectory()`). Sample/Debug content lives in `E:/Blunder Projects/Test` (`BLUNDER_DEFAULT_PROJECT_ROOT`). The engine checkout does not ship `Assets/`, `Resources/`, or `project.blunder`.
...
2. Teach Editor Session to honor `--project-root`; document Projects directory (`E:/Blunder Projects`)
