...
Current Import still packages companion animations under the Mesh (`Models/{mesh}/companions/` + `companion_animation_sources`), which teaches authors that clips belong to a Mesh. Product law ([ADR 0028](../../../docs/adr/0028-animation-clip-independent-of-mesh.md)) says AnimationClip is independent — like Unreal Anim Sequences vs Skeletal Mesh. Docs already flipped; code must catch up so Task 1 / DogWalk Import stop producing Mesh-child Intermediate.
