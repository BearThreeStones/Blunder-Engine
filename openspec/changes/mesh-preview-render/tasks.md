...
- [x] 1.4 Tests: Final preferred over Intermediate; Intermediate used when Final missing; failure returns clear error (placeholder allowed upstream)
...
- [x] 2.3 On successful regenerate, cache stores 3D still (invalidate/replace old texture-based Mesh cache entries via existing mtime rules)
- [x] 2.4 Tests: Mesh thumbnail path prefers 3D still; Texture path unchanged; failure → Mesh placeholder not base-color success
