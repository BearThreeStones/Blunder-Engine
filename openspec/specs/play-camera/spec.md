...
Given zero or more Camera Components in a scene, the engine SHALL resolve the Play view camera by preferring any camera with `isMain`, otherwise the first valid camera in stable iteration order (ascending EntityId). Empty input SHALL yield a not-ok result.
