...
bool DotNetHost::resolveProbeTickCount(const std::filesystem::path& script_host_dll,
  ...
  m_get_message_probe_count = nullptr;
  m_get_message_probe_last_id = nullptr;
  ...
  const char_t* message_count_name = L"GetMessageProbeCount";
  const char_t* message_last_id_name = L"GetMessageProbeLastId";
#else
  const char_t* type_name = "Blunder.ScriptHost.HostExports, Blunder.ScriptHost";
  ...
  const char_t* message_count_name = "GetMessageProbeCount";
  const char_t* message_last_id_name = "GetMessageProbeLastId";
#endif
  ...
  fn = nullptr;
  rc = load_and_get(dll_path.c_str(), type_name, message_count_name,
                    UNMANAGEDCALLERSONLY_METHOD, nullptr, &fn);
  if (rc != 0 || fn == nullptr) {
    out_error = "Failed to resolve HostExports.GetMessageProbeCount code=";
    out_error += intToEastl(rc);
    return false;
  }
  m_get_message_probe_count = reinterpret_cast<GetProbeTickCountFn>(fn);

  fn = nullptr;
  rc = load_and_get(dll_path.c_str(), type_name, message_last_id_name,
                    UNMANAGEDCALLERSONLY_METHOD, nullptr, &fn);
  if (rc != 0 || fn == nullptr) {
    out_error = "Failed to resolve HostExports.GetMessageProbeLastId code=";
    out_error += intToEastl(rc);
    return false;
  }
  m_get_message_probe_last_id = reinterpret_cast<GetProbeTickCountFn>(fn);
  return true;
}
...
int DotNetHost::getMessageProbeCount() const {
  if (m_get_message_probe_count == nullptr) {
    return -1;
  }
  return m_get_message_probe_count();
}

int DotNetHost::getMessageProbeLastId() const {
  if (m_get_message_probe_last_id == nullptr) {
    return -1;
  }
  return m_get_message_probe_last_id();
}
...
int DotNetHost::getMessageProbeCount() const { return -1; }

int DotNetHost::getMessageProbeLastId() const { return -1; }
