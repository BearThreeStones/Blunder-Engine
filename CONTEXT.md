...
**Frame graph pass**:
A node in the Frame graph: one GPU stage that declares reads and writes on Resources. Forward (and the Player) still use one Scene Pass whose callback calls `ForwardRenderPath::renderFrame` ([ADR 0067](docs/adr/0067-frame-graph-viewport-wire.md)). Editor Deferred uses a G-buffer Pass then a Lighting Pass instead of one Scene; those callbacks are slices of `DeferredRenderPath`, not `renderFrame` ([ADR 0069](docs/adr/0069-frame-graph-deferred-split.md)). Post-Scene overlay stages are more Passes on that graph; Copy is the Sink ([ADR 0068](docs/adr/0068-frame-graph-viewport-overlays.md)). Not a Vulkan render pass object, not a Job, and not a Secondary command buffer.
_Avoid_: `VkRenderPass` as this; a Job as a Pass; putting Camera Preview on the viewport graph this slice; treating the G-buffer images as this Pass
...
The mesh shading path that writes opaque geometry into a G-buffer, then lights that buffer in screen space into the offscreen color. Blend-transparent draws still use the Forward Render Path after that lighting, into the same offscreen color. Distinct from the Forward Render Path, from clustered or tiled deferred lighting, and from a visibility buffer. Editor viewport dispatch through Frame graph execute is a G-buffer Pass then a Lighting Pass; the G-buffer images stay path-owned and are not Frame graph resources. Decision record: [ADR 0062](docs/adr/0062-deferred-render-path.md). Dispatch: [ADR 0069](docs/adr/0069-frame-graph-deferred-split.md). Overlay and copy Passes on that graph: [ADR 0068](docs/adr/0068-frame-graph-viewport-overlays.md).
_Avoid_: Deferred lighting as this path; clustered lighting as this; visbuffer; importing G-buffer images as Frame graph resources; putting shadow or the post-lighting LOAD on their own Passes this slice; replacing the Forward Render Path as the editor default; Mesh Preview Render as this; Camera Preview as this; Scene Thumbnail Render as this; writing blend-transparent into the G-buffer; dropping transparent draws when the viewport uses this path; drawing scene overlays into the G-buffer; running scene overlays after transparent on this path; a second shadow map for this path; skipping the Directional shadow map when this path is on; dropping opaque skinned draws from the G-buffer; a product settings UI for enabling this path; the Player using this path via the editor opt-in; calling `renderFrame` as a leftover Scene callback when this path is on

**G-buffer**:
The extra textures the Deferred Render Path’s geometry pass writes, besides the viewport offscreen depth: albedo, world-space normal, metallic, roughness, AO, unlit, two-sided, and G-buffer receiver id. World position is reconstructed from that depth. Not the viewport offscreen color, not the pick ID target, and not the outline ID buffer. Not a Frame graph resource this slice ([ADR 0069](docs/adr/0069-frame-graph-deferred-split.md)).
_Avoid_: Offscreen color as this; treating viewport depth as a G-buffer plane; pick ID as a G-buffer plane; treating SSAO as this; expanding the viewport offscreen into a permanent MRT; an emissive plane; motion vectors; a shading-model id; a Blinn-Phong specular plane
