...
**Frame graph**:
A retained-mode CPU description of one frame of GPU work: Passes and named Resources with producer/consumer edges. Setup records virtual handles only; it does not allocate GPU memory or record commands. Distinct from Pipeline layout, from the Job System, and from Render Path internals. The caller owns the graph object. The main viewport reconstructs a graph each tick (Scene plus overlay and copy Passes); the type has no reset API and does not use a Frame arena. Decision record: [ADR 0061](docs/adr/0061-frame-graph-cpu-setup.md). External RHI attach: [ADR 0063](docs/adr/0063-frame-graph-resolve.md). Transient allocate: [ADR 0064](docs/adr/0064-frame-graph-allocate.md). Barrier plan: [ADR 0065](docs/adr/0065-frame-graph-barrier-plan.md). GPU Execute: [ADR 0066](docs/adr/0066-frame-graph-gpu-execute.md). Viewport Scene wire: [ADR 0067](docs/adr/0067-frame-graph-viewport-wire.md). Viewport overlays: [ADR 0068](docs/adr/0068-frame-graph-viewport-overlays.md).
...
**Frame graph handle**:
A virtual id for a Resource during Setup. Import may attach a non-owning RHI object to an External handle; reading that object back is Frame graph resolve and is Setup state. The handle remains the Setup identity. Not a Bindless table index and not a Secondary command buffer.
_Avoid_: `VkImage` / `ID3D12Resource` as this; bindless slot as this; Job data as this; treating the attached RHI pointer as the Setup identity

**Transient resource**:
A Frame graph Resource the graph creates and owns for one frame. Setup does not allocate GPU memory. After allocation, a live Transient has an owned RHI texture or buffer from a Frame graph allocator; a DCE’d Transient stays unallocated. Setup mutation destroys those owned objects. Tests use a stand-in allocator, not a Vulkan device. Not an External resource and not a Bindless table entry.
_Avoid_: Resource pool as this; treating a Bindless slot as graph-owned; a persistent offscreen target as this; treating Setup create as GPU allocation; requiring a Vulkan device to own a Transient in tests; allocating a DCE’d Transient; leaving owned Transient objects alive after Setup mutation

**External resource**:
A Resource created and kept outside the Frame graph and imported so the graph can track reads, writes, and barriers. Import attaches a non-owning RHI texture or buffer the caller already owns. Typical cases: the viewport offscreen color/depth, shadow map, history, readback. The graph does not allocate or free its GPU memory.
_Avoid_: Retained resource as the product name; Resource pool as a third kind; swapchain as an engine-owned surface (Slint still presents); treating import as creating GPU memory; `VkImage` as the import type in this slice

**Frame graph pass**:
A node in the Frame graph: one GPU stage that declares reads and writes on Resources. Forced-Forward editor Viewport still uses one Scene Pass whose callback calls `ForwardRenderPath::renderFrame` ([ADR 0067](docs/adr/0067-frame-graph-viewport-wire.md)). Editor Deferred and the Player use a G-buffer Pass then a Lighting Pass instead of one Scene; those callbacks are slices of `DeferredRenderPath`, not `renderFrame` ([ADR 0069](docs/adr/0069-frame-graph-deferred-split.md)). Post-Scene overlay stages are more Passes on that graph; Copy is the Sink ([ADR 0068](docs/adr/0068-frame-graph-viewport-overlays.md)). Not a Vulkan render pass object, not a Job, and not a Secondary command buffer.
_Avoid_: `VkRenderPass` as this; a Job as a Pass; putting Camera Preview on the viewport graph this slice; treating the G-buffer images as this Pass; treating the fragment shading rate image as this Pass

**Frame graph allocator**:
The object `allocate()` asks for an owned RHI texture or buffer given a Frame graph resource desc. The caller passes it as an argument each `allocate()`; the graph does not store it. A sticky second `allocate()` still takes the argument and does not call it. `createTexture` / `createBuffer` return `unique_ptr`; the graph takes ownership; resolve returns a raw pointer. A null `unique_ptr` for a live Transient fails that allocate as `AllocFailed` and rolls back every Transient object created in that call. Tests use a stand-in. This slice does not use `IRenderDevice` or a Vulkan device as this.
_Avoid_: Storing the allocator on the Frame graph; treating `IRenderDevice::createTextureFromAsset` as this; `VulkanAllocator` as this slice’s test stand-in; treating import as this; returning a raw pointer plus a separate destroy; treating a null create as a successful allocate

**Frame graph allocate**:
The phase after a successful compile that asks a Frame graph allocator for an owned RHI texture or buffer for each live Transient, from that resource’s Frame graph resource desc. DCE’d Transients are not allocated. External resources are not allocated. A successful allocate stays valid until Setup mutates: a second `allocate()` is Ok, does not call the allocator again, and does not replace the objects. It does not compile, does not run Execute callbacks, does not alias, does not insert barriers, and does not record commands. If compile has never succeeded, the last compile was not `Ok`, or Setup changed since that compile, `allocate()` returns `NotCompiled`, calls the allocator for nothing, and does not throw. If any live Transient create returns null, `allocate()` returns `AllocFailed`, destroys every Transient object from that call, and does not throw. Tests pass a stand-in allocator; this slice does not create a Vulkan device. Not compile and not Execute.
_Avoid_: Packt `compile()` creating Vulkan images as this; treating Setup `createTransient` as this; allocating External memory; allocating a DCE’d Transient; `VkImage` as the allocator’s product type; folding this into compile or execute; implicit compile inside allocate; reallocating on every `allocate()` while Setup is unchanged; leaving a partial Transient set after `AllocFailed`

**Frame graph compile**:
The phase that turns Setup into an executable order: producer/consumer edges, topological sort, dead-Pass elimination from Sinks, and a Resource lifetime on each live Resource. On one Frame graph handle, edges follow Setup order (Pass add order, then that Pass’s read/write call order): RAW (Write → later Read), WAW (Write → later Write), and WAR (Read → later Write). Usage does not grow extra edges. Same-Pass accesses never add an edge. Across Passes, a Read takes an edge from the previous Write; a Write takes an edge from the previous Write and from every Read after that previous Write. Read→Read has no edge. This slice has no Resource version type. It does not allocate GPU memory, alias heaps, insert barriers, or record commands. It does not throw. When Setup or the live subgraph is invalid, compile returns one failure and an empty live order, in this order: `InvalidPass` (a Pass handle that was never added), then `InvalidDesc` (a created or imported Resource whose Frame graph resource desc is illegal), then `InvalidImport` (an External resource imported with a null RHI pointer), then `DanglingAccess` (a Resource that was never created or imported), then `NoSink`, then `Cycle` (a cycle among Passes a Sink can reach). An illegal desc or a null import fails compile even if DCE would drop that Resource. Unreachable Passes are not a failure; DCE drops them, including unreachable Passes that cycle among themselves.
_Avoid_: Packt `compile()` creating Vulkan images and framebuffers as this phase; Execute as this; Job System scheduling as this; exceptions as the compile error channel; treating zero Sinks as a successful empty graph; a Resource version type in this slice; treating ColorAttachment Write as a hidden Read; treating an invalid Pass handle as a silent no-op; treating Setup-order ping-pong (A writes X / reads Y, B writes Y / reads X) as a live cycle; treating an illegal desc as `DanglingAccess`; treating a null import as `InvalidDesc` or `DanglingAccess`; ignoring an unused illegal desc or null import because DCE would drop it

**Frame graph execute**:
The phase that records each live Pass in compiled live order after a successful compile, a successful allocate, and a successful barrier plan. The caller passes a Frame graph recorder each `execute()`; the graph does not store it. For each live Pass, execute writes that Pass’s Frame graph barriers (rows whose `before` is that Pass) through the recorder, then runs the Pass callback with the same recorder. Callbacks are recorded with GraphBuilder `setExecute` as `void(IFrameGraphRecorder&)`. DCE’d Passes do not run. A live Pass with no callback is an empty run after its barriers. Execute does not compile, does not allocate, does not plan barriers, and does not mutate Setup. If the last compile was not `Ok`, or Setup changed since that compile, execute returns `NotCompiled` and records nothing. If compile is valid but allocate has not succeeded since that compile, execute returns `NotAllocated` and records nothing. If compile and allocate are valid but `planBarriers()` has not succeeded since that compile, execute returns `NotPlanned` and records nothing. Those checks run in that order: `NotCompiled`, then `NotAllocated`, then `NotPlanned`. Execute does not plan inside itself. The same successful compile, allocate, and plan may be executed more than once. Execute snapshots live Pass order, the barrier list, and callbacks at the start so a forbidden Setup mutation does not UAF; remaining Passes still record from that copy and execute still returns `Ok`. Callbacks must not throw and must not reenter; execute does not catch. `execute()` failure reasons describe graph state (`NotCompiled`, `NotAllocated`, or `NotPlanned` in this slice), not callback exceptions. Graph execute does not map Frame graph resource state onto `VkImageLayout`, does not call `vkCmdPipelineBarrier`, and does not begin, end, or submit a command buffer. The Vulkan Frame graph recorder outside the graph headers maps those rows and may call `vkCmdPipelineBarrier`. Execute is not a Job and does not replace Render Path internals. Viewport Scene dispatch through execute is [ADR 0067](docs/adr/0067-frame-graph-viewport-wire.md). Overlay and copy Passes on that graph are [ADR 0068](docs/adr/0068-frame-graph-viewport-overlays.md). Editor Deferred G-buffer and lighting Passes are [ADR 0069](docs/adr/0069-frame-graph-deferred-split.md). Decision: [ADR 0066](docs/adr/0066-frame-graph-gpu-execute.md).
_Avoid_: `vulkan.h` on the graph; expanding `ICommandList` as the recorder; storing the recorder on the graph; treating Execute as compile, allocate, or `planBarriers()`; replacing Forward internals with graph Passes; importing G-buffer images as Frame graph resources this slice; Job System scheduling Execute; a silent no-op when compile has not succeeded; treating a missing allocate as `NotCompiled`; treating a missing plan as `NotAllocated`; executing a stale live order after Setup mutation; implicit compile, allocate, or `planBarriers()` inside execute; catching callback exceptions into a `CallbackFailed` reason; requiring execute before reading an External RHI pointer; treating overlay and copy as hardcoded order after viewport execute

**Sink**:
A Frame graph pass marked with a side effect the graph cannot see (present, CPU readback, history export). Dead-Pass elimination walks backward from Sinks; a Pass that nothing reaches from a Sink is dropped with its Transient resources. The main viewport graph marks Copy as the Sink; Scene, G-buffer, and Lighting are not ([ADR 0068](docs/adr/0068-frame-graph-viewport-overlays.md), [ADR 0069](docs/adr/0069-frame-graph-deferred-split.md)). This glossary does not list every engine Sink.
_Avoid_: Treating every External resource as a Sink; treating the last writer of any resource as a Sink; treating the main viewport Scene as the Sink after overlay wire; treating the G-buffer Pass as a Sink; a hardcoded engine-wide Sink list in the graph library
...
**Frame graph resource desc**:
The CPU description of a Frame graph resource. `createTransient` and `importExternal` both take the same required desc type. Shape stays a separate argument; the desc does not repeat it. For a Texture: Frame graph format, extent (width and height as `uint32`, not `rhi::Extent2D`), sample count, mip count. For a Buffer: size in bytes. Setup stores it with the Frame graph handle. `resourceDesc` returns that stored desc without compile; DCE does not erase it. A handle that was never created or imported returns a default-constructed desc and does not throw. It is not a GPU allocation, not an RHI object, and not a Frame graph handle. Import attaches the RHI pointer as a separate argument from this desc. Clear, initial layout, usage flags, array layers, 3D, cube, and host-visible are not this desc.
_Avoid_: Packt `TextureHandle` on Setup; `rhi::BufferDesc` as the Setup identity; treating this desc as the attached RHI pointer; treating this desc as compile edges; a missing or default-empty desc on create or import; putting Pass clear or `read`/`write` usage on the desc; `VkFormat` or `rhi::PixelFormat` as this desc's format; requiring compile before reading the desc; using `rhi::Extent2D` on the desc

**Frame graph resolve**:
Reading the RHI texture or buffer on a Frame graph handle. For an External, the pointer is the non-owning import; that query is Setup state and does not need compile. For a Transient, the pointer is the graph-owned object from allocation; before allocation, and for a DCE’d Transient, it is null. A never-created handle, a shape mismatch, or a null import returns null and does not throw. DCE does not erase an External pointer. It is not GPU allocation, not a Frame graph handle, and not Execute command recording. Decision: [ADR 0063](docs/adr/0063-frame-graph-resolve.md). Transient allocate: [ADR 0064](docs/adr/0064-frame-graph-allocate.md).
_Avoid_: Treating resolve as allocate; `VkImage` as the resolved type; requiring execute before the readback; treating the pointer as the Setup identity; throwing on a missing or mismatched handle; clearing an External pointer because DCE dropped the Resource; treating a DCE’d Transient as allocated

**Frame graph format**:
The pixel format field on a Texture Frame graph resource desc. This slice names `Undefined` (invalid for a Texture), `R8G8B8A8_UNORM`, and `D32_SFLOAT`. It is not a `VkFormat` and not `rhi::PixelFormat`. A Buffer desc has no format.
_Avoid_: Storing `VkFormat` on Setup; expanding `rhi::PixelFormat` for this slice; treating Buffer size as a format; cataloging SSAO/outline/pick formats in this slice

**Resource access**:
A Pass Read or Write of a Frame graph resource, plus a usage: Sampled, ColorAttachment, DepthAttachment, or Storage. Create makes a Transient resource; import makes an External resource. Both record a required Frame graph resource desc. Import also attaches a non-owning RHI texture or buffer. Write is not create.
_Avoid_: Usage as a resource type; Packt input type as this; treating write as allocate

**Frame graph resource state**:
The CPU access state of a Frame graph resource at a Pass: the AccessKind (Read or Write) plus that access’s Usage, or Undefined before a Transient’s first live Pass. Across Passes, the leaving state is that Pass’s last access of the handle; the entering state is the next Pass’s first access of the handle. An External has no invented state before its first live access. Not a `VkImageLayout`, not a Shader resource layout, not a Pipeline layout, and not Frame graph format `Undefined`. Decision: [ADR 0065](docs/adr/0065-frame-graph-barrier-plan.md).
_Avoid_: `VkImageLayout` as this; treating Pipeline layout or Shader resource layout as this; treating Usage alone as this when Write→Write keeps the same Usage; using a Write in the same Pass as the leaving state when a later access on that Pass exists; treating an External’s first access as coming from Undefined; treating Texture format `Undefined` as this

**Frame graph barrier**:
A planned transition of one live Frame graph resource from one Frame graph resource state to another between consecutive live Passes that access that handle, or before a Transient’s first use. Texture and Buffer use the same rules. Same-Usage Read→Read across Passes is not a barrier. RAW, WAR, and WAW across Passes are barriers even when Usage stays the same. Same-Pass accesses are not barriers. `planBarriers()` records the row; Execute asks a Frame graph recorder to record it. The graph does not call `vkCmdPipelineBarrier`. Not a Job barrier. Decision: [ADR 0065](docs/adr/0065-frame-graph-barrier-plan.md).
_Avoid_: Job barrier as this; `vkCmdPipelineBarrier` as the graph’s product; treating compile or allocate as inserting this; treating a Shader resource layout change as this; requiring allocate before the plan; treating same-Usage Read→Read as this; treating a same-Pass read/write pair as this

**Frame graph barrier plan**:
The phase after a successful compile that records Frame graph barriers. Call `planBarriers()`. The recorded list is `barriers()`: each entry is a handle, from and to Frame graph resource state, the Pass just finished (`after`, invalid on a Transient’s first use), and the next live Pass (`before`). Order is `before` in live Pass order, then handle index. It does not allocate, does not run Execute callbacks, and does not call `vkCmdPipelineBarrier`. It does not require allocate. Without a successful compile, `planBarriers()` returns `NotCompiled` and `barriers()` is empty. A successful plan stays until Setup mutates; a second `planBarriers()` is Ok and does not rebuild. GPU Execute requires a successful plan; a missing plan is `NotPlanned`, not `NotAllocated`. Execute does not call `planBarriers()` internally. Not compile and not allocate. Decision: [ADR 0065](docs/adr/0065-frame-graph-barrier-plan.md).
_Avoid_: Folding this into `compile()` or `allocate()`; lazy-planning on `barriers()` or inside `execute()`; `vkCmdPipelineBarrier` as this; treating a missing plan as `NotAllocated`; Job barrier as this

**Frame graph recorder**:
The object `execute()` asks to record each Frame graph barrier; Pass callbacks then use the same object. The caller passes it each `execute()`; the graph does not store it. The interface method is `pipelineBarrier` on a Frame graph barrier row. Tests of the graph use a stand-in that logs those rows. The Vulkan Frame graph recorder (separate compilation unit, not `frame_graph.h`) maps rows to `vkCmdPipelineBarrier` on a bound PRIMARY. Not `ICommandList`, not a Vulkan command buffer as the type, and not a Frame graph allocator. Decision: [ADR 0066](docs/adr/0066-frame-graph-gpu-execute.md), [ADR 0067](docs/adr/0067-frame-graph-viewport-wire.md), [ADR 0068](docs/adr/0068-frame-graph-viewport-overlays.md).
_Avoid_: Expanding `ICommandList` as this; storing this on the graph; `vkCmdPipelineBarrier` on the graph; `begin` / `end` / `submit` on this; draw or `beginRenderPass` on this; treating the allocator as this

**GraphBuilder**:
The C++ API that records Frame graph Setup: create a Transient with shape plus a required Frame graph resource desc; import an External with that same desc type plus a non-owning `IGpuTexture*` or `IGpuBuffer*` (shape follows the pointer); read, write, Sink marks, and `setExecute` callbacks as `void(IFrameGraphRecorder&)`. Tests and later engine Setup both use it. `read`, `write`, `markSink`, and `setExecute` on a Pass handle that was never added make later compile return `InvalidPass`. Any of those Setup calls after a successful compile dirties that compile, that allocate, and that barrier plan: graph-owned Transient RHI objects are destroyed; External pointers are not; Transient resolve returns null until compile and allocate succeed again; `barriers()` is empty until compile and `planBarriers()` succeed again. Not a JSON parser, not Execute, and not the Job System.
_Avoid_: Packt `graph.json` as the product authoring format; a Slint or C# builder in this slice; treating compile as this API; Blackboard as handle passing in this slice; treating an invalid Pass handle as a silent no-op; treating Setup mutation as still-compiled or still-allocated; freeing an External pointer because Setup mutated; keeping `setExecute` as `void()` after GPU Execute
...
**Texture Loader**:
The Render-owned path that makes a color material texture sampleable without stalling the tick on disk decode or a GPU wait. CPU half is Jobs (file + decode into Job data). GPU half is an upload queue: staging pool, copy command buffers on the graphics queue, timeline poll, then Bindless `writeSlot`. Until that write, draws use the Bindless fallback index. It is not AssetManager, not Pull cook, not ThumbnailGenerator, not the Engine GPU cache, and not the Mesh Loader. v1 is textures only. Decision record: [ADR 0058](docs/adr/0058-async-texture-loader.md).
...
**Mesh Loader**:
The Render-owned path that makes unique Mesh Assets drawable without stalling first Iterate on disk read or a full unique-set `GpuMesh` upload. CPU half is Jobs (cooked `.meshbin` / Fast Path into Job data). GPU half is a per-tick `GpuMesh` budget on the RHI thread. MeshRenderers bind when that GUID is CPU-resident; draws skip until that GUID has a `GpuMesh`. Coalesce by Mesh Asset GUID; Job order is Scene Asset document order. Editor `openScene` and Play `engine_player` use the same `SceneSystem::loadScene` enqueue. The entity table stays synchronous. It is not Texture Loader, not entity paging, not per-instance glTF import, and not a loading-bar UI. Decision record: [ADR 0072](docs/adr/0072-async-scene-mesh-streaming.md).
_Avoid_: Streaming entities; Jobs calling RHI; wrapping `openGltfImportDocument` per instance as this; a DogWalk-only flag; frustum Job priority in this slice; binary Scene Asset in this slice; a first-frame millisecond gate; placeholder boxes for missing `GpuMesh`; treating Texture Loader checkerboard as Mesh Loader failure; Live editor pointers into Player

**Secondary command buffer**:
A Vulkan SECONDARY command buffer recorded on the RHI owner thread and executed inside a render pass by the frame PRIMARY (`RENDER_PASS_CONTINUE` + inheritance). One pool lives on the Vulkan device, with distinct in-flight slots for Viewport, Camera Preview, and Immediate (Mesh Preview) streams. Shadow, forward (opaque / scene overlay / transparent), deferred G-buffer and deferred lighting (editor Viewport, Player, Camera Preview, Mesh Preview, Thumbnail when those surfaces use the Deferred Render Path), outline, overlay lines, overlay AA, SSAO, and screen overlays use this path. GPU pick and Texture Loader copies stay PRIMARY. v1 does not record from Jobs or a render worker pool. Decision record: [ADR 0059](docs/adr/0059-secondary-command-buffers.md).
...
**Forward Render Path**:
The mesh shading path that lights each opaque and transparent draw while rasterizing into the offscreen color and depth. Distinct from the Deferred Render Path. Path internals are not Frame graph Passes; the viewport Scene callback may call this path ([ADR 0067](docs/adr/0067-frame-graph-viewport-wire.md)). Overlay and copy Passes on that graph are [ADR 0068](docs/adr/0068-frame-graph-viewport-overlays.md). Editor Deferred G-buffer and lighting Passes: [ADR 0069](docs/adr/0069-frame-graph-deferred-split.md).
_Avoid_: Clustered lighting as this; G-buffer as this path's output

**Deferred Render Path**:
The mesh shading path that writes opaque geometry into a G-buffer, then lights that buffer in screen space into the offscreen color. Blend-transparent draws still use the Forward Render Path after that lighting, into the same offscreen color. Distinct from the Forward Render Path, from clustered or tiled deferred lighting, and from a visibility buffer. Editor Viewport and Player dispatch through Frame graph execute is a G-buffer Pass then a Lighting Pass; Camera Preview / Mesh Preview / Thumbnail record the same path into dedicated offscreens off that graph. The G-buffer images stay path-owned and are not Frame graph resources. Placement Preview stays Forward. `BLUNDER_EDITOR_DEFERRED=0` forces only the editor Viewport back to Forward. Decision record: [ADR 0062](docs/adr/0062-deferred-render-path.md). Image-based VRS on the lighting pass: [ADR 0074](docs/adr/0074-image-based-variable-rate-shading.md). Dispatch: [ADR 0069](docs/adr/0069-frame-graph-deferred-split.md). Overlay and copy Passes on that graph: [ADR 0068](docs/adr/0068-frame-graph-viewport-overlays.md).
_Avoid_: Deferred lighting as this path; clustered lighting as this; visbuffer; importing G-buffer images as Frame graph resources; putting shadow or the post-lighting LOAD on their own Passes this slice; writing blend-transparent into the G-buffer; dropping transparent draws when the viewport uses this path; drawing scene overlays into the G-buffer; running scene overlays after transparent on this path; a second shadow map for this path; skipping the Directional shadow map when this path is on; dropping opaque skinned draws from the G-buffer; a product settings UI for enabling this path; a second Forward VRS path instead of converting Player / Preview / Thumbnail to this path; calling `renderFrame` as a leftover Scene callback when this path is on; Placement Preview as this path

**G-buffer**:
The extra textures the Deferred Render Path’s geometry pass writes, besides the viewport offscreen depth: albedo, world-space normal, metallic, roughness, AO, unlit, two-sided, and G-buffer receiver id. World position is reconstructed from that depth. Not the viewport offscreen color, not the pick ID target, and not the outline ID buffer. Not a Frame graph resource this slice ([ADR 0069](docs/adr/0069-frame-graph-deferred-split.md)). Not a fragment shading rate image.
_Avoid_: Offscreen color as this; treating viewport depth as a G-buffer plane; pick ID as a G-buffer plane; treating SSAO as this; expanding the viewport offscreen into a permanent MRT; an emissive plane; motion vectors; a shading-model id; a Blinn-Phong specular plane; the VRS rate image as this

**Variable rate shading**:
Image-based fragment rate on the clustered-deferred **lighting** fullscreen triangle: a previous-frame Sobel luminance-edge map attached with `VK_KHR_fragment_shading_rate`. Edges (`G > 0.1`) stay 1×1; flats go 2×2. The lighting shader does not change. Optional device extension; missing capability skips VRS (log, 1×1, no FATAL), matching `VK_EXT_mesh_shader`. `BLUNDER_EDITOR_VRS=0` force-off. Never on Forward, G-buffer, fog, SSAO, transparent, overlay, or pick. Player / Camera Preview / Mesh Preview / Thumbnail become Deferred so this lighting pass exists; Placement Preview stays Forward. Not per-draw, not per-primitive, not 4×4, not software VRS, not foveated VR. Decision record: [ADR 0074](docs/adr/0074-image-based-variable-rate-shading.md).
_Avoid_: Per-draw or per-primitive rate as this knife; VRS on G-buffer; a second Forward VRS path; software compute VRS; requiring the extension on Headless / Linux CI; a product settings page; 4×4

**Fragment shading rate image**:
A path-owned `R8_UINT` image, one slot per in-flight buffer like the G-buffer, holding packed fragment sizes for the next lighting pass (`0` = 1×1, `1 << 2 | 1` = 2×2). Written by graphics-queue Sobel compute after lighting; read as a RenderPass2 shading-rate attachment. Not Bindless, not an Asset, not Cook output, not a Frame graph Transient, not the G-buffer, not Hi-Z.
_Avoid_: Bindless slot as this; importing this as a Frame graph resource this slice; storing HDR color in this image; using this as the occupancy heatmap

**VRS mask overlay**:
Editor Viewport session toggle that colours the fragment shading rate image for acceptance (one colour 1×1, another 2×2; Packt Figure 9.4). Default off, not persisted, not Player, not Preview, not Thumbnail. Distinct from the froxel occupancy heatmap.
_Avoid_: Persisting this in Project settings; drawing it in Player; treating occupancy heatmap as this

**G-buffer receiver id**:
A compact index of the MeshRenderer that wrote that pixel, stored in the G-buffer so lighting can apply Light linking. Meshlets inherit their MeshRenderer’s id; they do not get a unique id per Meshlet. Not a raw EntityId bit pattern, not the pick ID, and not the outline packed `ob_id`. CPU-path draws still fit the Forward mesh draw cap. GPU-driven static opaque may use a wider slot than that cap.
_Avoid_: Reusing pick ID as this; reusing outline `ob_id` as this; storing EntityId bits as the GPU identity; a render layer mask as this; a unique id per Meshlet

**Deferred light list**:
The Light enabled candidates uploaded to the Deferred Render Path lighting pass for one frame. This slice uploads at most **32**, in stable EntityId order. Per-pixel shading still applies Light linking and the Light evaluation cap. Distinct from the Light evaluation cap.
_Avoid_: Treating this list’s size as raising the Light evaluation cap; a global first-8 that ignores linking

**Forward mesh draw cap**:
The maximum mesh draws the CPU record path records per list per frame for skinned, blend-transparent, pick, outline, and Preview/Thumbnail. It is a limit of per-draw constant slots (256). GPU-driven rendering is not this cap. The Bindless texture table does not raise it.
_Avoid_: Treating bindless textures as unlimited draws; applying this cap to GPU-driven static opaque; a second cap that counts unique textures as this limit

**GPU-driven rendering**:
The geometry submission path where the GPU culls Meshlets and writes draw commands. Static opaque and alpha-clip MeshRenderers use it by default in the editor viewport and Player, including the Directional shadow pass. Task and mesh shaders run when the device has `VK_EXT_mesh_shader`; otherwise compute culls and traditional vertex/fragment indirect draws. Skinned, blend-transparent, pick, outline, Mesh Preview, Camera Preview, and Scene Thumbnail stay on the CPU draw list. Shading still uses the Forward or Deferred Render Path. Not Bindless, not a visibility buffer, and not a per-scene demo flag. Decision record: [ADR 0070](docs/adr/0070-gpu-driven-rendering.md).
_Avoid_: Bindless texture table as this; Nanite; visbuffer; treating Preview or Thumbnail as this path in this slice; skinned meshlets; a Sponza-only scene switch; a dedicated compute queue as this

**Meshlet**:
A cook-time cluster of a mesh (at most 64 vertices and 124 triangles) with a bounding sphere and facing cone. It is the unit of GPU culling and mesh-shader work, stored in the cooked mesh Final. Not a MeshRenderer and not a Nanite cluster.
_Avoid_: Submesh as this; calling a whole MeshRenderer a meshlet; Nanite cluster; building meshlets on the tick
...
**Object Active**:
The persisted local on/off of an Object (Unity `activeSelf`). Default on. Toggling it on a parent does not rewrite descendants' Object Active. An inactive Object stays in the Hierarchy and the Scene Asset. Distinct from Light enabled, SkeletonModifier enabled, and from Delete (tombstone).
_Avoid_: Scene Visibility; Hierarchy eye as this; using tombstone or Entity.enabled as this flag; Light enabled; writing every descendant's flag when the parent toggles; calling this hidden or visibility overlay

**Active in Hierarchy**:
An Object is Active in Hierarchy when it is **Object Active** and every ancestor is Object Active. Only then it draws, is viewport-pickable, shows outline/gizmo, contributes Light, participates in Play Camera resolve, and Ticks Behaviours. It is not a stored field. Hierarchy still lists the row. Main Camera that is not Active in Hierarchy is not a Play camera — no Editor Camera fallback.
_Avoid_: Storing Active in Hierarchy on the Scene Asset; treating a locally Active child under an inactive parent as drawing or Ticking; Scene Visibility as this; using Editor Camera when the resolved Camera is inactive
...
The Edit Mode key A, while the pointer is over the Hierarchy Panel (docked or floating) and a selection exists, that sets **Object Active** on that selection to one shared value: off when every selected Object is Active, otherwise on (mixed selection turns on). Same operation as the **Hierarchy Active checkbox**. Viewport, Inspector, and Content Browser A do not toggle. Inline Rename types the letter A.
_Avoid_: Scene Visibility eye as this; Editor Camera strafe A as this; toggling when the pointer is over the viewport; treating A as Hierarchy Delete; flipping each selected Object independently; rewriting descendants' Object Active

**Hierarchy Active checkbox**:
The checkbox on a Hierarchy entity row for that Object's **Object Active**. Same operation as **Hierarchy Active toggle**. The row name is greyed when the Object is not **Active in Hierarchy**.
_Avoid_: Scene Visibility eye; Light enabled as this checkbox; greying only locally inactive rows so children under an inactive parent stay full-color

**Inspector Active checkbox**:
The checkbox on the Inspector identity row (beside the Object name) for **Object Active**. Same field and Document History Command as the Hierarchy checkbox and **Hierarchy Active toggle**. Multi-select mixed shows an indeterminate state; clicking aligns the selection the same way A does.
_Avoid_: A second Active flag on MeshRenderer; Light enabled as this checkbox
...
**Startup cover**:
The branded surface a windowed Editor Session shows from the first moment that session can show an OS window until the Editor Shell is on screen. That moment is after the Project File can yield the **Project display name**, still before cook and Vulkan. It occupies that same session window — not a second splash. That window stays a normal OS window: close aborts boot and ends the session with no confirm; minimize and resize stay available; the cover fills the client. Cook and other boot work run behind it. There is no minimum on-screen time: when the Shell is on screen, the cover is gone. This cover is not an error UI: a fatal boot still ends the process as today (the last stage name may remain until exit); there is no Retry and no error dialog on the cover. The field is Editor Theme Window (Base 2) with the Application Bar wordmark centered (`Blunder Editor - {Project display name}`) and a short English stage name under it. Visible stage names are **Cooking assets**, **Preparing editor**, and **Starting editor** as boot proceeds. It is not a percentage, not elapsed seconds, not a splash illustration, and not a Logo image. It is not the Viewport, not a Player splash, not Project Manager chrome, and not **Editor open progress**. An empty Viewport after the Shell is up is not this cover. Decision: [ADR 0052](docs/adr/0052-startup-cover-same-window.md).
_Avoid_: Splash as the product name (reads as a game/Player splash); treating Headless / CLI / MCP as having a cover; keeping the cover up until the Live scene or first 3D frame; waiting to show any window until after cook; covering only the empty HWND after the window already existed; calling the black HWND the intended look; a fake percent bar as this surface; elapsed seconds on this surface; a second splash window; locking close or resize while the cover is up; cinematic black or near-black fill; a splash illustration or separate Logo asset; Scene name or dirty `*` on the cover wordmark; a minimum dwell after the Shell is ready; an error page, Retry, or dialog on the cover

**Editor open progress**:
The Slint **Editor modal** overlay a windowed Editor Session shows on the Editor Shell after the **Startup cover** is gone, until that session can Iterate: Content Browser index `refresh` (scan) has finished and the startup scene entity table is instantiated. Shape is Godot’s project-init red box: title `Opening editor`, a discrete percent bar, an English stage caption (**Indexing content**, **Opening scene**), and elapsed whole seconds counted from the first OS window of this session. Percent is weighted completed boot stages, including cover stages already done — it does not restart cook at 0%, and it does not lerp. The Shell (Application Bar, docks, Viewport grid) may already be visible under the dim. Close while the overlay is up ends the Editor Session; there is no Cancel or Retry. Mesh Loader, Texture Loader, and thumbnail 2/tick MAY still run after dismiss. Headless / CLI / MCP, Project Manager, and Player do not show this overlay. Later Content Browser scene opens do not show it. It is not a second splash window and not a percent painted on the Startup cover. Decision: [ADR 0073](docs/adr/0073-editor-open-progress-overlay.md).
_Avoid_: Waiting for unique Mesh `GpuMesh`, texture GPU copies, or thumbnail drain; a second HWND; putting percent or elapsed on the Startup cover; Project Manager / Player / Headless chrome; Godot “global class names”; a smooth fake crawl; Cancel/Retry; treating Mesh Loader forest-grow as overlay leftover; reusing this modal when opening another scene mid-session
...
**Editor Camera fly**:
WASD / Q / E movement of the **Editor Camera** (Shift sprints) while right or middle mouse was pressed in the editor viewport and is still held. Middle-mouse pan still runs. Mouse capture may leave the viewport rect; fly continues until that button is released. Pointer over the viewport without that button does not fly. Not **Gameplay Input**.
_Avoid_: WASD fly from Hierarchy or Inspector; fly with only hover over the viewport; requiring the captured pointer to stay inside the viewport rect; treating this as Player Move; Q/E fly without the same button hold
...
**Fog Component**:
A native scene Component (like Camera / Light) on an entity: pose follows that entity’s TRS; world **Z** of that entity is the fog height origin (Z-up). It is the authored fog in the Scene Asset, not a C# Behaviour, and at most one per entity (Unique attachment). **Add… Fog** does not create a bound Object. Missing Fog means no **volumetric fog** (existing scenes stay clear). Fields: Fog enabled (default on), volumetric fog enabled (default on), Fog density (default 0.02), Fog height falloff (default 0.2), Fog view distance (default 60 m, positive), Fog albedo (linear RGB, default white), scattering distribution `g` in `[-0.99, 0.99]` (default 0.2). Fog enabled off or volumetric fog enabled off contributes no volumetric fog. When more than one Fog exists, the first Fog that is Fog enabled, volumetric on, Active in Hierarchy, and has view distance > 0 wins in stable EntityId order; others are ignored this slice. This slice does not store a second height-fog term, a box volume, or a 2D height-fog pass flag. Distinct from **volumetric shadows**. There is no Create… Fog and no New Scene Fog.
_Avoid_: Fog-as-Behaviour; a hidden process-global fog rig when the scene has no Fog Component; Y-up height; a second 2D analytical height-fog pass; Create… Fog; New Scene Fog; volumetric shadows as this Fog Unique

**volumetric fog**:
Participating media on the existing Viewport present path (editor Deferred lighting or Player Deferred lighting). Camera-aligned 3D froxels of about **16 px** screen tiles × **64** exponential Z slices (Unreal `CalculateGridZParams`, **S = 32**), near offset 9.5 cm through Fog view distance. Density at each cell is `ρ · exp2(−falloff · (world.z − fogHeight))`. One unshadowed illuminating Directional injects with Henyey–Greenstein `g` and shadow factor 1; point/spot/area do not inject. Thin temporal on pre-integrated scatter is about **20% current / 80% history**; skip out-of-frustum history; no `max(history, current)` keep. Composite after lighting on presented color: `color * T + inscatter` (reverse-Z clear 0 uses the far slice). Editor Viewport and Player share that composite; Camera Preview, Placement Preview, Mesh Preview, and Scene Thumbnail stay unfogged. No new GBuffer. Volume 3D textures stay off the Bindless color table.
_Avoid_: Camera Preview / Placement Preview / Mesh Preview / Scene Thumbnail fog; clustered local-light inject; volumetric shadows; a 2D height-fog fullscreen pass; a new GBuffer for fog; Bindless color-table entries for the volume

**volumetric shadows**:
Occlusion of in-scatter inside the fog volume (shadow maps, virtual shadow pages, or extra volume-shadow rays). This slice does not implement them: Directional inject uses shadow factor 1.
_Avoid_: Treating unshadowed directional inject as volumetric shadows; sampling VSM / classic shadow maps to darken froxels this slice
...
**Light shadows**:
Occlusion produced by a Light Component whose **Light contribution** includes shadows. This slice casts from **Directional** (one virtual clipmap / VSM, or the classic 1024² ortho map on devices without mesh shaders), **Point** (cubemap, at most 8 per view), and **Spot** (2D perspective, at most 8 per view). At most one Directional casts per view — the first Light enabled Directional whose contribution includes shadows, in stable EntityId order. Other Directionals still add direct light when their contribution includes illumination. Area does not cast. Shadows only on Point/Spot occludes without adding radiance; Shadows only on Area has no effect. Casters are opaque cooked meshlets (no alpha / skinned / foliage). Viewport and Player share this recording; Camera Preview stays unshadowed (ADR 0018).
_Avoid_: Area shadow maps; putting Point/Spot into the directional VSM; Nanite visbuffer / SMRT; treating `BLUNDER_EDITOR_SHADOWS` as the Viewport on-gate (it is an optional debug *off* switch: `0` disables editor shadows); Bindless color-table entries for shadow images
...
_Avoid_: A global first-8 that ignores linking; overflow by intensity or selection; treating the cap as a forever product limit; a 9th light stealing the Directional shadow map; Deferred lighting that ignores Light linking
...
**Clip Play**:
An AnimationTree script operation that names a **Clip Binding** logical name and **replaces the tree base** with that one clip. **v1 is a hard cut** — no fade duration. It does not insert on the **Fire slot**, does not add an Add2 layer, and does not auto-return when the clip ends. After the last key, pose **holds that key** and the **Clip Play override** stays until **Travel** / **Start**. Travel / Start clear the replacement and drive StateMachine again (including `Travel` to the remembered current state). **Fails with no mutation** when the name is empty, the logical name is not a complete Clip Binding, or the AnimationTree is not active — it does not auto-activate. C# may still spell the method `Play`; the product term is Clip Play. Not **Play** (the Player session). **v1 ship:** AnimationTree runtime, C-ABI, and Blunder.Api, plus engine tests. In the **Animation Window**, **Play** issues **Clip Play** of **Preview clip**; changing **Preview clip** issues the same Clip Play (Playing continues; Paused and Stopped do not start transport). Binding a Tree issues that Clip Play of the default **Preview clip** at clock 0 without starting transport. Edit preview chrome, not a second Unique. Soft cuts (snapshot or mixer fade, per-edge fade), unified BlendSpec, Canvas edits, and DogWalk content Play bars remain follow-ups, not Clip Play v1.
...
**Preview clip**:
The session-selected Clip Binding logical name in the **Animation Window** dropdown. **Play**, changing this dropdown, and binding a Tree **Clip Play** this name; window **Fire** inserts the same name on the **Fire slot**. Not a serialized AnimationTree field and not Document History.
_Avoid_: Fire target as the name of this control; a second Clip Play dropdown beside Fire; treating Preview clip as a Scene or Tree Asset field

**Animation Window**:
Persistent docked chrome for **Edit animation preview** on the current single selection's **AnimationTree**. Transport and timeline scrub advance that tree; the ruler is the **base dominant clip** clock (insert clip while Fire/OneShot occupies; **Clip Play override** clip while that override is set and Fire is not occupying). The dock stays open when unbound (no selection, multi-select, or no Tree) with transport and timeline disabled. Changing Hierarchy selection ends preview on the previously bound tree (ruler to 0, clear Fire slot, clear **Clip Play override**, halt transport, **End CINE**) then binds the new single Tree selection or the empty state; an unbound previous tree does not keep preview-advancing. Binding a Tree selects that tree's default clip as **Preview clip**, activates the tree if inactive, **Clip Plays** that name at clock 0, and does not start transport. **Stop** itself also **End**s CINE. The **End** control exits CINE only (clears in-CINE and input-suppression marks) without seeking or clearing Fire slot or Clip Play override. **Enter** is the CINE session mark only — it does not Fire a clip. Window **Loop** is Edit preview wrap of the current ruler clip while Playing; with Loop off, reaching the end Pauses on the last frame and leaves the tree active. **Stop** seeks the ruler to 0, clears the **Fire slot** insert, **End**s CINE, and leaves the tree active; it does not clear **Clip Play override**. **Play** activates an inactive tree before advancing, then **Clip Plays** **Preview clip**; Play from last-frame Pause starts again at 0. Loop is not a durable flag on **AnimationClip** and not AnimationPlayer loop. v1 also shows the clock source's logical name, **TimeScale** (the same AnimationTree field as Inspector — dual-track, document-dirty; the only Animation Window control on **Document History**), Edit **CINE** enter-end plus in-CINE and input-suppression marks, and a **Preview clip** dropdown of that tree's existing Clip Binding logical names. **Preview clip** is session preview: **Play** uses it for **Clip Play**; **Fire** inserts the same name on the **Fire slot**. Changing **Preview clip** **Clip Plays** the new name immediately (hard cut, clock 0) whether Playing, Paused, or Stopped. Playing continues; Paused and Stopped stay without starting transport. If the **Fire slot** is occupying, the insert stays; only the Clip Play base underneath changes, and the ruler still follows the insert until Fire ends. It does not Travel, author bindings, or dirty the document. Transport **Play / Pause / Stop / Loop**, **Fire**, and **Enter / End CINE** follow **Icon-first chrome**. Transport, playhead, Loop, Fire slot occupancy, Clip Play override, **Preview clip**, and CINE marks are session preview and do not dirty the document. Window **Fire** inserts on that tree's **Fire slot**; it is not Sync Group `fireSameName`. There is no second Clip Play dropdown beside Fire. The timeline is a ruler and playhead plus **Clip anatomy**. Replaces the overlay preview toolbar. Defaults to a bottom dock under the viewport (Godot Animation-panel rhythm) as its own dock panel kind; authors may retile it. Clip Binding authorship, Travel/BlendSpace/Add2, **AnimationTree Canvas**, and multi-object Sync Group Edit stay elsewhere.
_Avoid_: AnimationPlayer as host; S0/S1/BW/Fd; Canvas inside this window; keyframe/track authorship; Clip Binding rows in this window; Travel/BlendSpace/Add2 in this window; Camera Preview pin; Godot auto-reveal/hide; a second overlay toolbar; a playhead tied to **Preview clip** while Fire/OneShot occupies a different insert clip; window Fire as Sync Group fireSameName; a Sync member list in this window; Loop as an AnimationClip Asset field; AnimationPlayer.m_loop as the preview wrap; Stop deactivating the tree; a preview-only TimeScale that is not the Tree field; keeping v1 as a viewport overlay instead of a bottom dock; putting transport/playhead/Loop/Fire/CINE/Preview clip on Document History; continuing preview on a tree after selection leaves it; leaving CINE marks on after Stop or rebind; Enter CINE as a Fire; a second Clip Play dropdown beside Fire; Play ignoring **Preview clip** and Clip Playing a hidden default instead; changing **Preview clip** while Playing only storing the name until the next Play; changing **Preview clip** while Paused or Stopped only storing the name until the next Play; binding a Tree only filling **Preview clip** without Clip Play; calling **Preview clip** Fire target; Stop clearing a **Clip Play override**

**Clip anatomy**:
The read-only Animation Window listing of the **clip tracks that already exist** on the current ruler **AnimationClip**, plus those tracks' key times on the timeline. Rows are grouped by bone name; each group lists that bone's existing **clip tracks**. Bone groups follow the bone's first appearance in the clip; within a group the channel order is Position, then Rotation, then Scale, omitting channels the clip does not have. Missing TRS channels are omitted (no Skeleton × Position/Rotation/Scale fill). Channel rows show the Position / Rotation / Scale **Editor Icon** plus that word. Bone group titles are the bone name only (no Skeleton glyph). Key times are drawn as diamonds on that clip-track's lane; diamonds are not selectable or draggable. A session filter hides bone groups whose names do not contain the query; it does not dirty the document or the **AnimationClip**. Bone groups are collapsible, default expanded; fold state lives in this Editor Session only and re-expands when the ruler clip changes. Clip-track bones that are not on the bound **Skeleton** still form groups; this pass does not badge or hide them. The playhead is one vertical line through the ruler and the currently visible **clip track** rows. Press or drag on the timeline column (ruler and key lanes) seeks with the existing ruler rule; the name, filter, and fold column does not seek; diamonds are not a separate hit target. **Method tracks** are not this surface. Not clip authorship and not a second play host.
_Avoid_: Inserting, deleting, or retiming keys or tracks in this window; mute/enable, interpolation, wrap, or delete controls as this surface; Godot AnimationPlayer as host; treating Object Transform tracks as clip tracks; synthesizing empty clip tracks for unanimated bones; Method track rows or method-key diamonds in Clip anatomy; a Skeleton3D or scene-tree root as the group parent; Skeleton-hierarchy or alphabetic reorder of bone groups; a selected or dragged key diamond; filtering by channel word (Position / Rotation / Scale); persisting the bone-name filter or bone-group fold on the Scene or clip; hiding or warning on clip bones missing from the Skeleton; seeking from the name, filter, or fold column

**Edit animation preview**:
Authorship-viewport pose preview without Behaviour Tick. Chrome is the **Animation Window**. Always through the co-located **AnimationTree** (Travel / Start, **Fire slot**, named BlendSpace drives, TimeScale; **Clip Play** of **Preview clip** via **Play** and via changing Preview clip). No AnimationPlayer-only clip scrub. No Tree, or Tree inactive, means preview fails — same as Play.
